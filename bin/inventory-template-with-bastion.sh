...
INVENTORY_TEMPLATE=${HOME}/main/bin/inventory-base-with-bastion.json
...
extract_tfstate_bastion_instances_to_ansible_inventory_hosts() {
  jq '.resources[] | select(.type == "aws_instance")
  | .instances[].attributes
  | select(.tags.Components | contains("bastion"))
  | {
      (.tags.Name): {
        ansible_host: .public_ip,
        private_ip: .private_ip,
...
inject_ansible_bastion_hosts_json() {
  jq -c '.' \
  | while read line; do
      echo "| .bastion.hosts += ${line}"
    done
}
...
process_jq_template() {
  ...
  jq "$(extract_tfstate_kakfa_instance_to_ansible_inventory_hosts | inject_ansible_kafka_hosts_json \
    ...
    && extract_tfstate_schemaregistry_instances_to_ansible_inventory_hosts | inject_ansible_schemaregistry_hosts_json \
    && extract_tfstate_bastion_instances_to_ansible_inventory_hosts | inject_ansible_bastion_hosts_json)" ${INVENTORY_TEMPLATE}
}
